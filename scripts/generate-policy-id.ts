 ...
 * 
 * Usage:
 *   node scripts/generate-policy-id.js
 * 
 * Requirements:
...
async function generatePolicyId() {
  console.log('🔐 Generating NFT Policy ID...\n');

  // Validate environment
  if (!WALLET_SEED_PHRASE) {
    console.error('❌ Error: WALLET_SEED_PHRASE not found in environment');
    console.error('   Please set it in services/api/.env.local');
    process.exit(1);
  }

  if (!BLOCKFROST_PROJECT_ID) {
    console.error('❌ Error: BLOCKFROST_PROJECT_ID not found in environment');
    console.error('   Please set it in services/api/.env.local');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`📡 Connecting to Cardano ${CARDANO_NETWORK}...`);
    ...
    console.log('📜 Creating native script policy...');
    ...
    // Display results
    console.log('✅ Policy ID Generated Successfully!\n');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    ...
    for (const envFile of envFiles) {
      ...
      if (fs.existsSync(envPath)) {
        ...
        if (content.includes('NFT_POLICY_ID=')) {
          ...
          content += `\n# NFT Policy ID (generated by scripts/generate-policy-id.ts)\nNFT_POLICY_ID=${policyId}\n`;
          console.log(`   ✓ Added to ${envFile}`);
        }
        
        fs.writeFileSync(envPath, content, 'utf-8');
      } else {
        console.log(`   ⚠ Skipped ${envFile} (file not found)`);
      }
    }
    ...
    console.log(`💾 Policy script saved to: scripts/policy-script.json\n`);
